...
function drawText(props) {
  ...
  var legend = d3.legendColor()
    .scale(colorScale);
 d3.select(selector).append('g').attr("transform", "translate(550,10)").attr("class", "label2")
	.call(legend);
 ...
 legend = d3.legendColor()
    .scale(colorScale)
	.cells(2);
 d3.select(selector).append('g').attr("transform", "translate(100,40)")
	.call(legend);
		}
...
function render(props) {
  ...
  var link = svg.append('g').style('stroke', '#aaa').selectAll('line')
  .data(links)
  .enter()
  .append('line')
  .style('stroke-width', function (d) {
    return linkWidthScale(d.weight*1.3);
  })
  .on("click", function (d)
                 {  	
							var tmp=d3.select(this).style("stroke");
							if (tmp==='rgb(31, 120, 180)'){
								d3.select(this).style("stroke", "#ec1e1e");
							if((!paths.includes(d.source.name))){
								paths.push(d.source.name);
							}
							if((!paths.includes(d.target.name))){
								paths.push(d.target.name);
							}
  ...
  var label = node.append('text').text(function (d) {
    ...
    return Math.max(Math.min(2 * nodeRadiusScale(d.linkWeightSum), (1.6 * nodeRadiusScale(d.linkWeightSum) - 8) / this.getComputedTextLength() * labelTextScalingFactor),
    // Math.min(
 ...
 // drawText({
  ...
  d3.select('div#div0').append('div').attr('id', 'slider-container');
  ...
  function drawSliderControl(props) {
    ...
    function update(sliderValue) {
	  ...
	  document.getElementById("output").innerText=sliderValue;
	  //value.nextElementSibling.value = sliderValue
